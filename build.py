...
# Fallback offset for static generation (GMT+5)
...
for i in range(7):
    ...
    for m in day_matches:
        ...
        with open(f"{m_path}/index.html", "w", encoding='utf-8') as mf:
            ...
            # 1. Plain text for Meta Tags/Titles (to avoid the ">" syntax error)
            m_html = m_html.replace("{{DATE}}", m_dt_local.strftime("%d %b %Y"))
            m_html = m_html.replace("{{TIME}}", m_dt_local.strftime("%H:%M"))
            
            # 2. HTML Spans for visible display (User's device will update these)
